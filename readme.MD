...
4. `solve_vmg.sh` : Main bash file that runs pnp_par.py while attaching necessary rate model data or calculating it using [`ElectrchemicalKinetics.jl`](https://github.com/BattModels/ElectrochemicalKinetics.jl). It makes directories and appends necessary paths in the config file. Can specify number of cores for parallelization.
...
6. `tutorial_pnp/` : Tutorial for solving PNP equations using simple Butler-Volmer rates. Main solve contained in `notebook.ipynb`. Employs original mesh `mesh3D.xml` from [Nature Chemistry paper](https://www.nature.com/articles/s41557-021-00865-1).   
