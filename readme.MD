...
* $`\phi = \phi_{o}`$ at $`z=0`$, $` 0 \le r \le a_{s}`$
...
* $`J_{o} = -D_{o}\frac{\partial c_{3}}{\partial n} = D_{r}\frac{\partial c_{4}}{\partial n} = c_{3}k_{red} - c_{4}k_{ox}`$
...
$$i = F\int_{0}^{a_{s}} F\int_{0}^{2\pi} J_{o}(r, \theta) r \text{d}r\text{d}\theta$$
