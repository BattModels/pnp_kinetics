...
center = center_domain(bvec1, domain='AA')
...
save_sol(sol_dir+'/kred_f', vec_red, mesh)
save_sol(sol_dir+'/kox_f', vec_ox, mesh)
save_sol(sol_dir+'/Vdl_f', vec_Vdl, mesh)
...
th_list = np.linspace(0,2*np.pi,500)
...
if rank==0:
    ...
    print('Current = {}'.format(F*np.trapz(y, x=r_list)))
