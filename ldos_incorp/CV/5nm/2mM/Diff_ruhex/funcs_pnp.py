...
def center_domain(coords, bvec1, domain='AA'):
    # Arguments: Coordinates, basis vector and domain center
    center_aa = [0,0]
    center_ab = [2*bvec1[0]/3, 0]
    center_sp = [bvec1[0], 0]
    if domain == 'AA':
        center_id = np.argmin(norm(coords.T - center_aa, axis=1))
    elif domain == 'AB':
        center_id = np.argmin(norm(coords.T - center_ab, axis=1))
    elif domain == 'SP':
        center_id = np.argmin(norm(coords.T - center_sp, axis=1))
    center = np.array(coords[:,center_id]) # Center of nanopipette
    return center_id, center
